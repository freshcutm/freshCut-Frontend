    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-0">
      ...
      <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 to-purple-500 text-white p-4 mb-6 shadow">
        <div class="flex items-center gap-3">
          ...
          <div class="font-semibold text-white">Resumen rápido</div>
          <span class="ml-auto text-sm text-white">Últimos {{ months.length }} meses</span>
        </div>
        <div class="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
          <div class="bg-white/10 rounded px-3 py-2">
            <div class="text-white">Cortes</div>
            <div class="text-lg font-bold text-white">{{ totalCuts12m }}</div>
          </div>
          <div class="bg-white/10 rounded px-3 py-2">
            <div class="text-white">Gasto total</div>
            <div class="text-lg font-bold text-white">{{ formatCOP(totalSpend12m) }}</div>
          </div>
          <div class="bg-white/10 rounded px-3 py-2">
            <div class="text-white">Promedio mensual</div>
            <div class="text-lg font-bold text-white">{{ formatCOP(avgMonthlySpend) }}</div>
          </div>
          <div class="bg-white/10 rounded px-3 py-2">
            <div class="text-white">Costo promedio</div>
            <div class="text-lg font-bold text-white">{{ formatCOP(avgCostPerCut) }}</div>
          </div>
        </div>
      </div>
